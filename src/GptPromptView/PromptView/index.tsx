...
export const PromptView: React.FC = () => {
  ...
  return (
    <form
      ...
      onKeyDown={enterSubmitHandler(submitHandler)}
    >
      {flip && (
        <>
          <TextField
      ...
      <Autocomplete
        ...
        renderOption={(props, option) => (
          <Box component="li" {...props} key={option.id}>
            <pre style={{ margin: "4px", minHeight: "1rem" }}>
              {option.value}
            </pre>
          </Box>
        )}
        ...
        onChange={async (event, _prompt) => {
          ...
          const { options: _options } = await chrome.storage.sync.get({
          ...
          await chrome.storage.sync.set({
            options: { ...options, promptOptions: newState },
          });
        }}
        ...
        <FormControlLabel
          ...
          control={
            <Switch
              size="small"
              checked={flip}
              onChange={(_, value) => setFlip(value)}
            />
          }
          label={"flip prompt"}
