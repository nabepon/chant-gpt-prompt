...
export const GptPromptViewForContentScript: React.FC<{
  content: string;
  removeView: () => void;
}> = (props) => {
  ...
  if (!state.isMounted) {
    return (
      <div style={{ fontSize: "10px", whiteSpace: "nowrap" }}>loading...</div>
    );
  }
  if (state.mountError) {
    return null;
  ...
  return (
    <Box
      onMouseDown={() => setZIndex(Number(Date.now().toString().slice(5, 10)))}
      sx={{ zIndex, position: "relative" }}
    >
      <Draggable handle=".handle">
        <div>
          <Card sx={{ minWidth: 256, width: 520, resize: "horizontal" }}>
            <Box
              className="handle"
              sx={{ display: "flex", paddingLeft: "4px", minHeight: "37px" }}
              onDoubleClick={toggleExpand}
            >
              <Button
                size="small"
                onClick={onClickTab("prompt")}
                variant={state.tab === "prompt" ? "outlined" : "text"}
              >
                Prompt
              </Button>
              <Button
                size="small"
                onClick={onClickTab("answer")}
                variant={state.tab === "answer" ? "outlined" : "text"}
              >
                Chat
              </Button>
              <Button
                ...
                variant={state.tab === "history" ? "outlined" : "text"}
              >
                History
              </Button>
              {state.tab === "answer" && (
  ...
  return (
    <StatusIconButton
      ...
      onClick={() => {
        ...
        const history = historyState.historyRecord.histories.find(
          (history) => history.id === state.id
        );
        if (!history) return;
